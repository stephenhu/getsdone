...
function prependComment( id, data ) {
  ...
  comment.className = "comment col-lg-6 col-md-6 col-sm-6";
  ...
  text.innerHTML = response.msg.comment + " (now)";
...
function cache() {
  ...
  if( sessionStorage != null ) {
    ...
    if( action != null && action.value.length > 0 ) {
      sessionStorage.setItem( "cached_action", action.value );
    }
    else {
      sessionStorage.setItem( "cached_action", "" );
    }
...
function read_cache() {
  ...
  if( sessionStorage != null ) {
    ...
    cached = sessionStorage.getItem("cached_action");
